...
// ./nona3 -o out -m TIFF_m /src/test/t.pto /src/test/
void mainFunc(){
    std::string basename;
    ...
    const char* scriptFile = "/src/test/t.pto";
    ...
    pano.setImageFilename(0, "/src/test/image0.jpg");
    pano.setImageFilename(1, "/src/test/image2.jpg");
...
//         printf("%d\n",__LINE__);
//         outputImages = HuginBase::getImagesinROI(pano, pano.getActiveImages());
//         for(std::set<unsigned int>::iterator it=outputImages.begin() ;it!=outputImages.end();it++)
//         {
